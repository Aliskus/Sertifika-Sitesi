...
$haberler = mysql_query('Select * from haberler where durumu=1 '.(!empty($ara)?' and  baslik like "%'.mysql_real_escape_string($ara).'%" or  icerik like "%'.mysql_real_escape_string($ara).'%" ':'').' order by eklendigi_tarih desc');
?>
        ...
        <div class="row">
            ...
            <div class="col-12">                
                <?php while($haber = mysql_fetch_array($haberler)){
                    echo '<div class="card mt-3"><div class="card-header"><a class="text-black" href="/sertifika/haber.php?id='.$haber['id'].'">'.kisalt($haber['baslik'],60).'</a></div>
                    <div class="card-body">'.kisalt(strip_tags($haber['icerik']),220).'</div>
                    </div>';
                }
                ?>
            </div>
